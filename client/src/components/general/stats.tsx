...
export function Stats({ year }: StatsProps) {
  ...
  const statisticsJson = useMemo(() => {
    ...
    try {
      const result = statistics.safeParse(
        JSON.parse(statisticsData.statistics),
      );
      if (!result.success) {
    ...
    <div className="p-6">
      ...
      <div className="flex flex-col gap-2 text-center">
        <span className="text-4xl font-semibold">
          {statisticsJson.countRegistrants}
        </span>
        <h3 className="text-lg text-muted-foreground titillium-web-bold">
          Total Registrants
        </h3>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[repeat(3,_1fr)] gap-4 my-8 p-2 justify-between">
        <StatsCard
          value={statisticsJson.femalePercent.toFixed(2) + "%"}
          label="Female Attendance"
        />
        <StatsCard
          value={statisticsJson.countSchoolsRepresented}
          label="Schools Represented"
        />
        <StatsCard
          value={statisticsJson.countCountriesRepresented}
