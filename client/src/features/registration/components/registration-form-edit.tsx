...
export type RegistrationFormEditValues = z.infer<typeof registrationFormSchema>;

export function RegistrationFormEdit() {
  const { control, handleSubmit } = useForm<RegistrationFormEditValues>({
    ...
    <form className="grid grid-cols-1 md:grid-cols-[auto,_1fr] gap-8" onSubmit={handleSubmit(onSubmit)}>
      <FormSection name="Registrant Information" description="This information allows us to update you with the latest information about MakeUC.">
        <FormGroup>
          ...
          <Input control={control} label="Last Name" name="lastName" placeholder="Enter Last Name" readOnly />
        </FormGroup>
        <Input control={control} label="Email" name="email" placeholder="Enter Email" readOnly />
        <FormGroup>
          <InputNumber control={control} label="Age" name="age" placeholder="Enter Age" readOnly />
      ...
      <FormSection name="Education" description="Based on your current academic institution and what degree you are working towards.">
        ...
        <FormGroup>
          <Input control={control} label="Major(s)" name="major" placeholder="Enter Major" readOnly />
      ...
      <FormSection name="Additional Details" description="All of these fields are optional and you can fill in as much or as little detail as you would like.">
        ...
        <InputNumber control={control} label="Number of Hackathons Attended" name="hackathonsAttended" placeholder="Enter Number of Hackathons Attended" readOnly />
      ...
      <div className="flex justify-center md:justify-end md:col-span-2">
        <Button type="submit">Submit Registration Update</Button>
      </div>
    </form>
  );
}
